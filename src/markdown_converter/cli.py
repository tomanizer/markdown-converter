...
def setup_cli_logging(
    verbose: bool = False, log_file: Optional[str] = None, structured: bool = False
) -> None:
    """
...
def load_config(config_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    env_config = {
        ...
        "preserve_structure": os.getenv("MDC_PRESERVE_STRUCTURE", "true").lower()
        == "true",
    }
...
def cli(
    ...
    # Log CLI startup
    logging.info(
        "Markdown Converter CLI started",
        extra={
            "verbose": verbose,
            "log_file": log_file,
...
@click.option(
    "--metadata/--no-metadata", default=True, help="Include document metadata"
)
@click.pass_context
def convert(
    ctx: click.Context,
...
@click.option(
    "--max-memory", default=2048, type=int, help="Maximum memory usage per worker in MB"
)
@click.option(
    "--file-size-limit",
    default=70,
...
def health(ctx: click.Context, json: bool) -> None:
    ...
    if json:
        ...
        click.echo(
            f"{status_emoji.get(health_status['status'], '❓')} System Health: {health_status['status'].upper()}"
        )
        ...
        if "performance" in health_status:
            ...
            click.echo(
                f"🧠 Memory Usage: {perf['memory_percent']:.1f}% ({perf['memory_used_mb']:.1f} MB)"
            )
...
@click.option(
    "--max-files", default=5, type=int, help="Maximum number of files to test"
)
@click.pass_context
def test(
    ctx: click.Context, input_dir: Path, output_dir: Optional[Path], max_files: int
) -> None:
